...
class ImageImport(ctk.CTkFrame):
    def __init__(self, parent, import_func, init_leds):
        ...
        self.cameras = ctk.CTkComboBox(self, values=self.unique_camera_info_list, variable=self.selected_camera).place(relx=0.5, rely=0.5, relwidth = 0.7, anchor = 'center')
        ctk.CTkButton(self, text = 'Подключиться', command=self.open_dialog).place(relx=0.5, rely=0.55, relwidth = 0.7, anchor = 'center')
    ...
    def drawLedContoursAndGetCenters(self, contours,img):
        ...
        for i in contours:
            area = cv2.contourArea(i)
            if area < median and abs(median - area) > 100:
                continue
            M = cv2.moments(i)
            if M['m00'] != 0:
                cx = int(M['m10']/M['m00'])
    ...
    def update_frame(self):
        with self.ia.fetch(timeout=np.float32(1)) as buffer:
            ...
            blur = cv2.GaussianBlur(grey_img, (11, 11), 
                        0)
            ret,thresh1 = cv2.threshold(blur,254,255,cv2.THRESH_BINARY)
            ...
            if self.led_points == len(contoursCenters):
                pcb_led_correct = True
            else:
                pcb_led_correct = False
            if pcb_led_correct:
                cv2.putText(img, str(pcb_led_correct), (0, 20),
